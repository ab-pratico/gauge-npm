...
export class GaugeDownloader {
  ...
  get url() {
    if (this.os === 'linux') {
      return options.GAUGE_URL_TEMPLATE_LINUX.replace('%GAUGE_VERSION%', options.GAUGE_VERSION)
  ...
  private installGaugeJsPlugin() {
    return new Promise<void>((resolve, reject) => {
      shelljs.exec(
        `${options.GAUGE_BINARY} --install js`,
        {
          async: true,
          env: {
  ...
  private installGaugeHtmlReportPlugin() {
    return new Promise<void>((resolve, reject) => {
      shelljs.exec(
        `${options.GAUGE_BINARY} --install html-report`,
        {
          async: true,
          env: {
            GAUGE_HOME: options.GAUGE_HOME,
